...
class SGD:
    ...
    def step(self) -> None:
        ...
        for i, param in enumerate(self.params):
            ...
            # Apply weight decay
            if self.lmda != 0:
                param.grad = param.grad + self.lmda * param.data
            if self.mu != 0:
                # Update the momentum buffer
                self.b[i] = self.mu * self.b[i] + param.grad
                param.grad = self.b[i]
...
class RMSprop:
    ...
    @t.inference_mode()
    def step(self) -> None:
        for b, v, p in zip(self.b, self.v, self.params):
            g = p.grad
            if g is None:
                continue
            # Apply weight decay
            ...
            v.copy_(self.alpha * v + (1 - self.alpha) * g * g)
            g = g / (v.sqrt() + self.eps)
            if self.mu != 0:
                # Update the momentum buffer
                b.copy_(self.mu * b + g)
...
class Adam:
    ...
    def step(self) -> None:
        for theta, m, v in zip(self.params, self.m, self.v):
            ...
            v.copy_(self.beta2 * v + (1 - self.beta2) * g * g)
...
class AdamW:
    ...
    def step(self) -> None:
        for theta, m, v in zip(self.params, self.m, self.v):
            ...
            if g is None:
                continue
            # Apply weight decay directly to the parameters (decoupled weight decay)
