...
class Encoder(nn.Module):
    def __init__(self, latent_dim_size: int, hidden_dim_size: int):
        ...
        self.linear1 = nn.Linear((7*7*32), hidden_dim_size)
        self.linear2 = nn.Linear(hidden_dim_size, latent_dim_size)

    def forward(
            self, x: Float[Tensor, "batch 1 height width"]
    ) -> Float[Tensor, "batch latent_dim_size"]:
        x = self.conv1(x)
        x = self.relu(x)
        x = self.conv2(x)
        x = self.relu(x)
        encoder_reshape = Rearrange("b c h w -> b (c h w)")
        x = self.linear1(encoder_reshape(x))
        x = self.relu(x)
        x = self.linear2(x)
        return x
...
class Decoder(nn.Module):
    def __init__(self, latent_dim_size: int, hidden_dim_size: int):
        super().__init__()
        self.decoder_linear1 = nn.Linear(latent_dim_size, hidden_dim_size)
        self.decoder_linear2 = nn.Linear(hidden_dim_size, (7*7*32))
        self.decoder_transconv1 = nn.ConvTranspose2d(32, 16, 4, 2, 1)
        self.decoder_transconv2 = nn.ConvTranspose2d(16, 1, 4, 2, 1)
        self.decoder_relu = nn.ReLU()

    def forward(
            self, x: Float[Tensor, "batch latent_dim_size"]
    ) -> Float[Tensor, "batch 1 height width"]:
        x = self.decoder_linear1(x)
        x = self.decoder_relu(x)
        x = self.decoder_linear2(x)
        x = self.decoder_relu(x)
        decoder_reshape = Rearrange("b (c h w) -> b c h w", c = 32, h = 7, w = 7)
        x = self.decoder_transconv1(decoder_reshape(x))
        x = self.decoder_relu(x)
        x = self.decoder_transconv2(x)
        return x
...
class Autoencoder(nn.Module):
    def __init__(self, latent_dim_size: int, hidden_dim_size: int):
        ...
        self.encoder = Encoder(self.latent_dim_size, self.hidden_dim_size)
        self.decoder = Decoder(self.latent_dim_size, self.hidden_dim_size)
    ...
    def forward(
        ...
        x_prime = self.decoder.forward(z)
        return x_prime
...
class AutoencoderTrainer:
    ...
    def train(self) -> Autoencoder:
        ...
        for epoch in range(self.args.epochs):
            ...
            for imgs, _ in pbar:
                loss = self.training_step(img=imgs)
                pbar.set_postfix(loss = f"{loss:.3f}", step = f"{self.step:06}")

            accuracy = self.log_samples()
...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim_size: int, hidden_dim_size: int):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Linear((7*7*32), self.hidden_dim_size),
            nn.ReLU(),
            nn.Linear(self.hidden_dim_size, 2*self.latent_dim_size),
            Rearrange("b (two latent) -> two b latent", two = 2)
        )
        self.decoder = nn.Sequential(

            nn.Linear(self.latent_dim_size, self.hidden_dim_size),
            nn.ReLU(),
            nn.Linear(self.hidden_dim_size, (7*7*32)),
            nn.ReLU(),
...
class VAETrainer:
    ...
    def training_step(
        ...
        loss_fn = nn.MSELoss()
        sigma = t.exp(logsigma)
        kl = ((sigma**2 + mu**2 - 1) / 2) - logsigma   # [batch, latent]
        reconstruction_loss = loss_fn(x_prime, img)
        kl_loss = kl.sum(dim=-1).mean()                 # scalar
...
class Generator(nn.Module):
    def __init__(
        ...
        self.hidden_layers = Sequential(
            nn.ConvTranspose2d(hidden_channels[-1], hidden_channels[-2], 4, 2, 1),
            BatchNorm2d(hidden_channels[-2]),
            ReLU(),
            nn.ConvTranspose2d(hidden_channels[-2], hidden_channels[-3], 4, 2, 1),
            BatchNorm2d(hidden_channels[-3]),
            ReLU(),
            nn.ConvTranspose2d(hidden_channels[-3], img_channels, 4, 2, 1),
            Tanh()
...
class Discriminator(nn.Module):
    def __init__(
        ...
        self.hidden_layers = Sequential(
            ...
            LeakyReLU(),
            Conv2d(hidden_channels[0], hidden_channels[1], 4, 2, 1),
            BatchNorm2d(hidden_channels[1]),
            LeakyReLU(),
            Conv2d(hidden_channels[1], hidden_channels[2], 4, 2, 1),
            BatchNorm2d(hidden_channels[2]),
            LeakyReLU()
        )
        self.classifier = Sequential(
            Rearrange("b c h w -> b (c h w)"),
            Linear(int(hidden_channels[-1]*(img_size//2**n_layers)**2), 1),
            Sigmoid()
        )
...
class DCGANTrainer:
    ...
    def training_step_discriminator(
        ...
        D_loss = -t.log(D_x).mean() - t.log(1-D_G_z).mean()
        D_loss.backward()
    ...
    def train(self) -> DCGAN:
        ...
        for epoch in range(self.args.epochs):
            ...
            for img_real, label in progress_bar:
                ...
                # Generate random noise
                noise = t.randn(self.args.batch_size, self.model.latent_dim_size).to(device)
                img_fake = self.model.netG(noise)
                img_real = img_real.to(device)
                D_loss = self.training_step_discriminator(img_real, img_fake)
                G_loss = self.training_step_generator(img_fake)

                # Update progress bar
                self.step += 1
                progress_bar.set_description(f"{epoch=}, {D_loss=:.4f}, {G_loss=:.4f}, batches={self.step}")
