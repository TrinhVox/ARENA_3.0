...
if not hasattr(torchvision.io, "VideoReader"):
    class VideoReader:  # harmless stand-in so the isinstance check resolves to False
        pass
...
# %%
# Step 3: Converts the logits into a distribution with softmax
...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        E_x = t.mean(residual, dim=-1, keepdim=True)
        var_x = t.var(residual, dim=-1, unbiased=False, keepdim=True)
        top = (residual - E_x)
        bottom = t.sqrt(var_x + self.cfg.layer_norm_eps)
        layer_norm = top/bottom
        layer_norm_scaled = layer_norm * self.w + self.b
        return layer_norm_scaled
...
# %%
# Exercise - Implement apply_casual_mask
...
class Attention(nn.Module):
    ...
    def apply_causal_mask(
        ...
        """
        _ , _, q_size, k_size = attn_scores.shape
        all_ones = t.ones(q_size, k_size, device=attn_scores.device)
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        # Dot product to create QK^T
        QK = einops.einsum(Q, K, "b sq n h, b sk n h -> b n sq sk")

        # Scaled and masked
        d_head = K.shape[-1]
        QK_scaled = QK/np.sqrt(d_head)

        QK_masked = self.apply_causal_mask(attn_scores=QK_scaled)

        attn_scores = t.softmax(QK_masked, dim=-1)

        # Step 2
        V = einops.einsum(normalized_resid_pre, self.W_V, "b sk e, n e h -> b sk n h") + self.b_V
        attn_pattern = einops.einsum(attn_scores, V, "b n sq sk, b sk n h -> b sq n h")
        result = einops.einsum(attn_pattern, self.W_O, "b sq n h, n h e -> b sq n e")
        attn_out = einops.einsum(result, "b sq n e -> b sq e") + self.b_O
        return attn_out
...
class MLP(nn.Module):
    ...
    def forward(self, normalized_resid_mid: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        attn_in = einops.einsum(normalized_resid_mid, self.W_in, "b s e, e m -> b s m") + self.b_in
        activated = gelu_new(attn_in)
        attn_out = einops.einsum(activated, self.W_out, "b s m, m e -> b s e") + self.b_out
        return attn_out
...
class DemoTransformer(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_vocab"]:
        ...
        final_logits = self.ln_final(residual)
        return self.unembed(final_logits)
...
class TransformerSampler:
    ...
    def sample_top_k(logits: Float[Tensor, "d_vocab"], k: int) -> int:
        ...
        """
        topk_prob, topk_ids = t.topk(logits, k)
        sampled_token = t.distributions.categorical.Categorical(logits=topk_prob).sample()
        return topk_ids[sampled_token].item()
